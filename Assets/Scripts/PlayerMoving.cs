...
public class PlayerMoving : MonoBehaviour {
    ...
    private void Move()
    {
        ...
        Vector3 moveDistance = (playerInput.moveZ * transform.forward + playerInput.moveX*transform.right) * moveSpeed * Time.deltaTime;
        if( playerAnimator.GetCurrentAnimatorStateInfo(0).IsName("run")==false && moveDistance != Vector3.zero)
        {
            playerAnimator.SetBool("idle", false);
            playerAnimator.SetBool("run", true);

        }
        else if (playerAnimator.GetCurrentAnimatorStateInfo(0).IsName("run") && moveDistance == Vector3.zero)
        {
